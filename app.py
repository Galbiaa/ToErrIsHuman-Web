...
if st.button("🔍 Calcola Rischio Errore", type="primary", use_container_width=True):
    if not (ax_file and co_file and sa_file):
        ...
        m_col1.metric(label="Stima Rischio Errore Diagnostico Umano", value=f"{mean_p_error:.1%}")
        m_col2.metric(label="Probabilità Patologia stimata dall'AI", value=f"{mean_p_ai:.1%}")

        if mean_p_error >= 0.5:
            st.error("🚨 **ALTO RISCHIO ERRORE DIAGNOSTICO**: L'AI indica una probabilità elevata che la diagnosi fornita dal rater contenga un errore. Si suggerisce un secondo parere radiologico.")
        else:
            st.success("✅ **BASSO RISCHIO ERRORE**: La valutazione fornita appare in linea con le caratteristiche delle MRI ed il profilo di rischio.")